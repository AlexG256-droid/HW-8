...
public class GameController {
  ...
  private void saveGame() {
    ...
    view.displayMessage("Game saved to " + SAVE_DIRECTORY);
  }
  ...
  private void loadGame() {
    try {
      ...
      String playerFile = chooseFile(SAVE_DIRECTORY, "Select Player File");

      this.map = LoadGameData.loadMap(gameFile);
      Player loadedPlayer = PlayerLoad.loadPlayer(playerFile, gameFile, this.map);
  ...
  private String chooseFile(String directory, String title) {
    ...
    if (!input.isEmpty()) {
      ...
      if (customFile.exists() && customFile.isFile()) {
        ...
        System.out.println("Invalid file. Using default.");
      }
    }
