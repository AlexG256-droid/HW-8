...
public class LoadGameData {
  ...
  /**
   * Loads the Map object (rooms, name, version) from the given JSON file.
   *
   * @param pathname The path to the game data JSON file.
   * @return A fully constructed Map object.
   * @throws IOException If reading the file fails.
   */
  public static Map loadMap(String pathname) throws IOException {
    ObjectMapper objectMapper = new ObjectMapper();

    // 1. Read the GameData object (which has name, version, items, etc.)
    ...
    // 3. Create the list of Room objects
    List<Room> roomList = parseRooms(roomsNode, gameData);

    // 4. Return a new Map object
    return new Map(roomList, gameData.getName(), gameData.getVersion());
  }

  /**
   * Parses the "rooms" array to construct a list of Room objects.
   */
  ...
  /**
   * Creates a single Room object from a JSON node.
   */
  ...
  /**
   * Finds a Puzzle by name in the gameData object.
   */
  ...
  /**
   * Finds a Monster by name in the gameData object.
   */
